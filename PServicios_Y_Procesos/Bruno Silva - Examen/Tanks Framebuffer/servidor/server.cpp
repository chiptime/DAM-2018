...
void addSizeIMG(Sprite sprite, Bullets *position_Bullet){
    switch((int) position_Bullet->dir){
        case 1://up
            position_Bullet->position.x += 100/4;//(int) sprite.image.width/2;
//            position_Bullet->position.y += 100/4;//(int) sprite.image.width/2;
            break;
        case 2://left
            position_Bullet->position.y += 96/4; //(int)sprite.image.height/2;
            break;
        case 3://down
            position_Bullet->position.x += 100/4;//(int) sprite.image.width/2;
            position_Bullet->position.y += 96/2; //sprite.image.height;
            break;
        case 4://right
            position_Bullet->position.x += 100/2;//sprite.image.width;
            position_Bullet->position.y += 96/4;//(int) sprite.image.height/2;
            break;
    }
}
...
void handle_client(int client_fd) {
    ...
    for(int i = 0; i <MAX_CLIENTS; i++){
        shared->tank[i].balas.position.x = 900;
        shared->tank[i].balas.position.y = 900;
    }
    do{
        ...
        for(int i = 0; i < MAX_CLIENTS; i++){
            if(clientes.id == i){
                ...
                if(tecla == 'b'){
                    shared->tank[i].balas.dir = teclas(&shared->tank[i], &shared->tank[i].balas, tecla);
                    addSizeIMG( shared->tank[i].sprite, &shared->tank[i].balas);
                }
	    }
            moveBullet(&shared->tank[i].balas, shared->tank[i].balas.dir);
        }
        crashBullet(shared);
